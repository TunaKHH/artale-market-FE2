...
export const MessageList = memo<MessageListProps>(({
      ...
      {Array.from({ length: 3 }).map((_, index) => (
        <div key={index} className="animate-pulse">
          <div className="flex items-start space-x-3">
            <div className="w-8 h-8 bg-gray-200 dark:bg-gray-700/30 rounded-full flex-shrink-0"></div>
            <div className="flex-1 space-y-2">
              <div className="h-4 bg-gray-200 dark:bg-gray-700/30 rounded w-3/4"></div>
              <div className="h-3 bg-gray-200 dark:bg-gray-700/30 rounded w-1/2"></div>
            </div>
          </div>
        </div>
      ))}
  ...
  return (
    <div className={`flex flex-col ${className}`}>
      ...
      {/* 訊息計數器 */}
      {messages.length > 0 && (
        <div className="flex items-center justify-between px-4 py-2 bg-gray-50/50 dark:bg-gray-800/20 border-t border-gray-200 dark:border-gray-700/50 text-sm text-gray-500 dark:text-gray-400">
